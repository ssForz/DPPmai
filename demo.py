...
samples.grid(row=3, column=2, pady=5)
 
out = Entry(
   frame,
)
out.grid(row=4, column=2, pady=5)

def Gen():
    global SAMPLES_PER_RAY
    SAMPLES_PER_RAY = int(samples.get())
    global output
    output = out.get()
    window.destroy()
...
if __name__ == '__main__':
    ...
    plt.imsave(output, rays_color.reshape(img_wh[1], img_wh[0], 3))